...
3. **Trailing politeness is ignored** (B106): `thank you`, `thanks`, `please`,
   `cheers`, and a few similar closers are stripped before matching so
   `"… over thank you"` / `"… that's all, thanks"` still finalize. Politeness
   alone is not an end phrase.
